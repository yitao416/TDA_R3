...
output_foler = gsub("k/","k_betti/",input_folder)
mainDir <- getwd()
...
################ bottleneck distance ################
folder <- "02_wholeDist/network/"
networktypeList <- c("top100")
dat <- c()
pat <- "([M])\\w+"
typeList <- c()
...
for (networktype in networktypeList) {
  ...
  network.f <- paste0(folder,"kyber_top100Rev2_distMD5.txt")
...
######### compare with the coreDist
orgFolder <- "02_wholeDist/network/"
org.f <- paste0(orgFolder,"kyber_top100Rev2_distMD5.txt")
org.betti <- read.table(org.f)[2:4] %>% as.matrix()


coreFolder = "01_coreDist/network/"
core.f <- paste0(coreFolder,"kyber_top100Rev_dist.txt")
core.betti <- read.table(core.f)[2:4] %>% as.matrix()
dat <- c()

bk0 <- bottleneck(org.betti, core.betti,dimension = 0)
bk1 <- bottleneck(org.betti, core.betti,dimension = 1)
bk2 <- bottleneck(org.betti, core.betti,dimension = 2)
bk3 <- bottleneck(org.betti, core.betti,dimension = 3)
bk.type <- str_extract(f,pat)
typeList <- c(typeList,bk.type)
dat <- rbind(dat,c(bk0,bk1,bk2,bk3,bk.type))
